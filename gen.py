...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--lang', default='ENG',
                        help='Select language : ENG/JPN/ZHTW')
    parser.add_argument('-i', '--iter', default=1,
                        help='no. of times to use the same image')
    parser.add_argument('-N', '--nostop', action='store_true', default=False,
                        help='no. of times to use the same image')
    args = parser.parse_args()
    INSTANCE_PER_IMAGE = int(args.iter)
    if args.nostop:
        while True:
            main(args.viz)
    else:
